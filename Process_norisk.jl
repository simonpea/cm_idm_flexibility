...
if case == "no_risk" 
    ...
    outputpath = "output/no_risk/full/"
...
function yearlyKPI(scen1, scen2, scen3)
    yearly_KPI = DataFrame()
    yearly_KPI.Value = ["ID Price", "Revenue CM (Mio. Euro)", "Revenue ID (Mio. Euro)", "Revenue total (Mio. Euro)", "Downshift for CM (TWh)", "Upshift for CM (TWh)", "Downshift for IDM (TWh)", "Upshift for IDM (TWh)", "Volume of Load Shifting (TWh)", "Average Value for CM (Euro per MWh)", "Average Value on ID (Euro per MWh)", "Average Value of Load Shifting (Euro per MWh)"]
    yearly_KPI[!, "Scenario 1"]= ["Index", sum(scen1.Hourly_Cashflow_CM)/1000000, sum(scen1.Hourly_Cashflow_ID)/1000000, sum(scen1.Hourly_Cashflow_Sum)/1000000, sum(scen1.Shift_Dn_CM)/1000000, sum(scen1.Shift_Up_CM)/1000000, sum(scen1.Shift_Dn_ID)/1000000, sum(scen1.Shift_Up_ID)/1000000,  sum(scen1.Shift_Up)/1000000, sum(scen1.Hourly_Cashflow_CM)/sum(scen1.Shift_Up_CM), sum(scen1.Hourly_Cashflow_ID)/sum(scen1.Shift_Up_ID), sum(scen1.Hourly_Cashflow_Sum)/sum(scen1.Shift_Up)]
    yearly_KPI[!, "Scenario 2"]= ["High", sum(scen2.Hourly_Cashflow_CM)/1000000, sum(scen2.Hourly_Cashflow_ID)/1000000, sum(scen2.Hourly_Cashflow_Sum)/1000000, sum(scen2.Shift_Dn_CM)/1000000, sum(scen2.Shift_Up_CM)/1000000, sum(scen2.Shift_Dn_ID)/1000000, sum(scen2.Shift_Up_ID)/1000000, sum(scen2.Shift_Up)/1000000, sum(scen2.Hourly_Cashflow_CM)/sum(scen2.Shift_Up_CM), sum(scen2.Hourly_Cashflow_ID)/sum(scen2.Shift_Up_ID), sum(scen2.Hourly_Cashflow_Sum)/sum(scen2.Shift_Up)]
    yearly_KPI[!, "Scenario 3"]= ["Low", sum(scen3.Hourly_Cashflow_CM)/1000000, sum(scen3.Hourly_Cashflow_ID)/1000000, sum(scen3.Hourly_Cashflow_Sum)/1000000, sum(scen3.Shift_Dn_CM)/1000000, sum(scen3.Shift_Up_CM)/1000000, sum(scen3.Shift_Dn_ID)/1000000, sum(scen3.Shift_Up_ID)/1000000, sum(scen3.Shift_Up)/1000000, sum(scen3.Hourly_Cashflow_CM)/sum(scen3.Shift_Up_CM), sum(scen3.Hourly_Cashflow_ID)/sum(scen3.Shift_Up_ID), sum(scen3.Hourly_Cashflow_Sum)/sum(scen3.Shift_Up)]
    rowdict = Dict()
    for row in 2:nrow(yearly_KPI)-3
    rowdict[row] = scen1_chance * yearly_KPI[row, "Scenario 1"] + scen2_chance * yearly_KPI[row, "Scenario 2"] + scen3_chance * yearly_KPI[row, "Scenario 3"]
    end
...
function Expected_df(scen1, scen2, scen3)
    ...
    for col in names(scen1)[valcol:end]
        expected_df[!, col] = scen1_chance * scen1[!, col] + scen2_chance * scen2[!, col] + scen3_chance * scen3[!, col]
    end
    return expected_df
end
...
daily_nodal_expected_df = Expected_df(daily_nodal_scen1_df, daily_nodal_scen2_df, daily_nodal_scen3_df)
...
daily_global_expected_df = Expected_df(daily_global_scen1_df, daily_global_scen2_df, daily_global_scen3_df)
...
yearly_KPI = yearlyKPI(scen1_df,scen2_df,scen3_df)
...
for col in names(yearly_nodal_values_scen1)[5:end]
    yearly_nodal_values_expected[!, col] = scen1_chance * yearly_nodal_values_scen1[!, col] + scen2_chance * yearly_nodal_values_scen2[!, col] + scen3_chance * yearly_nodal_values_scen3[!, col]
end
if writing
...
sum(yearly_nodal_values_expected.Shift_Up_CM)
sum(yearly_nodal_values_expected.Shift_Dn_CM)
